...
| `ashare-intraday-review` | 时间窗 / 手动 | 盘中快速检查：持仓异动、技术位互动温度计、宏观背景校准、资金流向/板块温度计 |
...
四个复盘技能统一引用 `references/technical-depth-guide.md`：

- **晚间复盘**（唯一全量跑技术引擎的时段）：大盘四大指数 + 全部持仓/自选股，产出四维深度卡片
- **盘中复盘**（不跑引擎）：用"技术位互动温度计"——拿盘中现价对照昨晚算好的支撑/压力位
...
1. 按当前时间命中时间窗 → 选出对应技能
2. 判断当天是否 A 股交易日（akshare 交易日历，支持退化为工作日判断）
